...
def _cigna_compartment_search_bundle(
    ...
    logical id returns an empty searchset.
    """
...
def _bundle_entry_dedupe_key(entry: Any) -> str:
...
def _fhir_merge_cigna_dual_patient_bundles(
    primary: dict[str, Any], secondary: dict[str, Any]
) -> dict[str, Any]:
    """Merge two searchset Bundles; de-dupe by resource id/fullUrl."""
    out = dict(primary)
    p_ent = primary.get("entry")
    s_ent = secondary.get("entry")
    if not isinstance(p_ent, list):
        p_ent = []
    if not isinstance(s_ent, list):
        s_ent = []
    seen: set[str] = set()
    merged_entries: list[Any] = []
    for e in p_ent + s_ent:
        k = _bundle_entry_dedupe_key(e)
        if k in seen:
            continue
        seen.add(k)
        merged_entries.append(e)
    out["entry"] = merged_entries
    if "total" not in out or out.get("total") is None:
        out["total"] = len(merged_entries)
    meta = out.get("meta")
    if not isinstance(meta, dict):
        meta = {}
        out["meta"] = meta
    ...
    tags.append(
        {
            ...
            "display": "Merged compartment results for two Cigna patient logical ids.",
        }
    )
    meta["tag"] = tags
    return out
