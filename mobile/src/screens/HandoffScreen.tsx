 ...
 */
function compartmentPatientQueryParam(tokenPatientId: string, patientPayload: unknown): string {
...
/**
 * Cigna: compartment searches (EOB, Coverage, …) use FHIR Patient.id when present; when it
 * differs from the token `patient` claim (e.g. enterprise vs evi-*), the proxy merges a second leg.
 * Uses merge_patient_id (alias: cigna_merge_patient_id on the server).
 */
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const patPayload = pat.ok ? pat.data : null;
        const compartmentQ =
          payerId === "cigna"
            ? cignaCompartmentQuery(patientId, patPayload)
            : `?patient_id=${compartmentPatientQueryParam(patientId, patPayload)}`;
        ...
        const enc = await fetchFhirJson(
          apiBase,
          `/api/fhir/${payerSeg}/Encounter/${compartmentQ}`,
          token
        );
        ...
        const cl = await fetchFhirJson(
          apiBase,
          `/api/fhir/${payerSeg}/Claim/${compartmentQ}`,
          token
        );
